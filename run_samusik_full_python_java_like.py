...
t0 = time.time()
model = XShift(k=20, metric="angular", root_merge=True, merge_mahalanobis=2.0, verbose=True)
labels = model.fit_predict(X)
seconds = time.time() - t0
out_dir = Path("data/samusik/results")
out_dir.mkdir(parents=True, exist_ok=True)
pd.DataFrame({"population_id": truth, "python_angular_k20_java_like": labels}).to_csv(out_dir / "python_samusik01_full_java_like_labels.csv", index=False)
summary = pd.DataFrame([{
    "run": "python_angular_k20_full_java_like",
    "n_cells": len(labels),
...
summary.to_csv(out_dir / "python_samusik01_full_java_like_metrics.csv", index=False)
